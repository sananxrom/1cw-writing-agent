...
const PROVIDER_URLS = {
  ...
  ollama:     null, // dynamic — uses OLLAMA_HOST env or localhost
}
...
export async function callAI({ provider = 'anthropic', model, apiKey, system, user, maxTokens = 2000 }) {
  // Resolve URL — Ollama uses env var or default
  let url = PROVIDER_URLS[provider]
  if (provider === 'ollama') {
    url = (process.env.OLLAMA_HOST || 'http://localhost:11434') + '/api/chat'
  }
  if (!url && provider !== 'ollama') throw new Error(`Unknown provider: ${provider}`)

  // Use env key as fallback if no user key provided
  ...
  if (provider === 'anthropic') {
    ...
    // OpenAI-compatible: openai, perplexity, qwen, groq, gemini
...
export async function stageExtract({ content, title, sourceUrl, sourceName, provider, model, apiKey }) {
  const system = `Extract key facts from article. Return only JSON. Be concise.`

  const user = `Source: ${(title || 'Article').slice(0, 120)}
${sourceName ? `From: ${sourceName}` : ''}

Content:
${(content || '').slice(0, 3000)}

Return ONLY this JSON (short values, no line breaks in strings):
{"headline_angle":"one sentence angle","key_facts":["fact1","fact2","fact3","fact4"],"why_it_matters":"one sentence","suggested_category":"${ALL_CATEGORIES[0]}"}`

  // No retries — single fast call, haiku should nail this
  return callAI({ provider, model, apiKey, system, user, maxTokens: 400 }).then(parseJSON)
}
...
Return ONLY valid JSON:
{
  ...
  return withRetry(() => callAI({ provider, model, apiKey, system, user, maxTokens: 2000 }).then(parseJSON), { label: 'write', retries: 1 })
}
...
Return ONLY valid JSON:
{
  ...
  return callAI({ provider, model, apiKey, system, user, maxTokens: 600 }).then(parseJSON)
}

// ── Stage 4: Regen single field ──────────────────────────────
export async function stageRegen({ field, instruction, article, writingPrompt, provider, model, apiKey }) {
  // Special case: image SEO fields (alt, title, caption all at once)
  if (field === 'imageAlt') {
    const system = `You are an SEO specialist. Generate image metadata for a WordPress featured image. Return only JSON.`
    const user = `Article title: ${article.title}
Category: ${article.primaryCategory || ''}
Focus keyword: ${article.focusKeyword || ''}

Generate image alt text, title, and caption for the featured image of this article.

Return ONLY valid JSON:
{
  "imageAlt": "descriptive alt text 10-15 words, include focus keyword naturally",
  "imageTitle": "image title 5-8 words",
  "imageCaption": "optional brief caption 1 sentence or empty string"
}`
    return withRetry(() => callAI({ provider, model, apiKey, system, user, maxTokens: 200 }).then(parseJSON), { label: 'regen:imageAlt' })
  }
