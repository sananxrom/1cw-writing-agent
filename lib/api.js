...
export async function wpRequest(endpoint, method = 'GET', body = null) {
  const profileSlot = typeof window !== 'undefined'
    ? (JSON.parse(localStorage.getItem('1cw_profile') || '{}').slot || '1')
    : '1'
  const res = await fetch('/api/wordpress', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ endpoint, method, body, profileSlot }),
  })
...
export async function generateArticle(params, { batchIndex = 0, batchDelay = 0 } = {}) {
  ...
  // Single endpoint call with maxDuration: 60 — this is what worked before the split
  const res = await fetch('/api/generate', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      ...params,
      writingProvider: wp.apiKey ? wp : null,
      editingProvider: ep.apiKey ? ep : null,
      batchIndex,
      batchDelay,
    }),
  })
  ...
  try { data = await res.json() } catch {
    throw new Error('Server returned non-JSON. Check Vercel logs.')
  }
...
export async function saveToWordPress(article, wpMeta) {
  ...
  // Save to DB articles table so Published/Drafted tabs show the article
  try {
    await fetch('/api/save-article', {
      ...
      body: JSON.stringify({
        ...
        tags: [...(article.regionTags||[]), ...(article.keywordTags||[])],
      }),
    })
  } catch {} // non-critical
